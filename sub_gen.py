...
if __name__ == '__main__':
    ...
    # Inference part
    # 저장된 모델로 예측을 진행합니다.
    # 데이터를 분리했을 시에 쓰는 코드입니다.
    # predictions = []
    # for k in range(total_k):  # 사용한 모델 종류 개수
    #     for i in range(3): # split한 데이터 개수만큼
    #         cur = k * 3 + i
    #         model = torch.load(f'./ensemble_models/model{cur}.pt')
    #         dataloader = Dataloader(model_names[k], args.batch_size, args.shuffle, args.train_path, args.dev_path,
    #                         args.test_path, args.predict_path)
    #         prediction = trainer.predict(model=model, datamodule=dataloader)
    #         predictions.append(torch.cat(prediction))

    #### 데이터 분리 안 하셨으면 아래 코드 쓰면 됩니다.####
    predictions = []
    for k in range(total_k):  # 사용한 모델 종류 개수
        model = torch.load(f'./ensemble_models2/model{k}.pt')
        dataloader = Dataloader(model_names[k], args.batch_size, args.shuffle, args.train_path, args.dev_path,
                        args.test_path, args.predict_path)
        prediction = trainer.predict(model=model, datamodule=dataloader)
        predictions.append(torch.cat(prediction))
...
#         predictions.append(torch.cat(prediction))
    ...
    predictions = list(round(max(min((float(i) + 1) * 2.5, 5), 0), 1) for i in predictions)
    ...
    output.to_csv(f'./submissions/output-{cur}.csv', index=False)
